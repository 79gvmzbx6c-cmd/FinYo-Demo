...
@st.cache_data(show_spinner="Running in-memory simulation...")
...
with tab_fhi:
    ...
    with st.container(border=True):
        ...
        with col_left:
            run_fhi_button = st.button(
                "Run Full Simulation (Baseline + Policy)", type="primary", key="run_fhi"
            )
        with col_right:
            st.write(
                f"This will run a demonstration for **7 Emirates × 16 sectors × {EMPLOYERS_PER_COMBO} employers** "
                "under **Baseline** and **Policy** to compare outcomes."
            )
    ...
    if not st.session_state.get("fhi_results"):
        st.info("Click **Run Full Simulation (Baseline + Policy)** to generate results.")
    else:
